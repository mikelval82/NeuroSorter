...
def run(spike_dict, current):
    ...
    for experiment in np.unique(spike_dict['ExperimentID']):
        ...
        trigger_timestamps = np.array([spike_dict['TimeStamps'][it] for it,ch in enumerate(spike_dict['ChannelID']) if ch == current['channelID'] and spike_dict['UnitID'][it] == trigger and spike_dict['ExperimentID'][it] == experiment])/fs
        unit_timestamps = np.array([spike_dict['TimeStamps'][it] for it,ch in enumerate(spike_dict['ChannelID']) if ch == current['channelID'] and spike_dict['UnitID'][it] == unit and spike_dict['ExperimentID'][it] == experiment])/fs
