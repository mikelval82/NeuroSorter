...
def run(spike_dict, current):
    ...
    fs = int(spike_dict['SamplingRate']  )
    spikes = [spike_dict['TimeStamps'][it] for it in current['plotted']]
    triggers = spike_dict['Trigger']
    ...
    for i in range(triggers.size - 1):
        datahist = datahist + [(stamp - triggers[i])/fs for stamp in spikes if stamp > triggers[i]+range_[0]*fs and stamp < triggers[i]+ range_[1]*fs]
    ...
    for i in range(triggers.size - 1):
        trial_raster = [(stamp - triggers[i])/fs for stamp in spikes if stamp > triggers[i]+range_[0]*fs and stamp < triggers[i]+ range_[1]*fs]
