...
class nev_manager:   
    ...
    def __alignment(self):
        ...
        for it,wave in enumerate(self.full_spike_dict['Waveforms']):
            if self.full_spike_dict['ExperimentID'][it] == self.ExperimentID:
                pos_min = np.argmin(wave[5:20])+5
                desplazamiento = reference-pos_min
                if desplazamiento < 0:
                    self.full_spike_dict['Waveforms'][it] = np.hstack((wave[abs(desplazamiento):],np.zeros((abs(desplazamiento),))))
                elif desplazamiento > 0:
                    self.full_spike_dict['Waveforms'][it] = np.hstack(( np.zeros((abs(desplazamiento),)), wave[:-abs(desplazamiento)], ))
