...
class GUI_behaviour(QMainWindow, ui):
    ...
    def update_amplitude_threshold(self):
        ...
        r_min = int(text.split(',')[0][2:])
        r_max = int(text.split(',')[1].split(']')[0])
        threshold = int(text.split(',')[2].split(']')[0])
        print('#######################################')
        print(r_min, r_max, threshold)
        [index, time_consumed] = self.dmg.clean_by_amplitude_threshold(r_min, r_max, threshold)
